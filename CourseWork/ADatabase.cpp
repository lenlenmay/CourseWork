...
void ADatabase::ParserFromFile(std::string str)
{
	...
	if (std::regex_match(str.c_str(), result, regular)) {
		...
		DataList.AddEnd(tempData);// поменять на список
	}	
}
...
{
	DeleteContentFromFile(this->PathToFile);// поменять на список
	for (int i = 0; i < DataList.Count(); i++) {
		InputInFile(FormContentForFile(DataList.GetElement(i)));
	}
}

void ADatabase::AddRecord()
{
